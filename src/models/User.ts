import { supabase } from '../db' // Importa o cliente Supabase configurado
...
class User {
    ...
    validate() {
        ...
        if (
            Number.isNaN(nivelConscienciaNum) ||
            nivelConscienciaNum < 0 ||
            nivelConscienciaNum > 5
        ) {
            // Validação do nível de conscientização
            errors.push(
                'Nível de conscientização deve ser um número entre 0 e 5.',
            )
        }
