...
class ReceitaController {
    async create(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            for (const subtema of subtemas) {
                const { data: subtemaData, error: subtemaError } =
                    await supabase
                        .from('subTema')
                        .select('*')
    ...
    async getAll(req: Request, res: Response): Promise<void> {
        try {
            const { data: receitas, error: receitasError } = await supabase
                .from('receitas')
                .select(
                    '*, correlacaoReceitas(tema, subtema), fotosReceitas(url)',
                )
            ...
            const receitasComDetalhes = await Promise.all(
                receitas.map(async (receita) => {
                    ...
                    receita.correlacaoReceitas?.forEach(
                        (correlacao: Correlacao) => {
                            if (correlacao.subtema)
                                subtemas.add(correlacao.subtema)
                        },
                    )
    ...
    async getById(req: Request, res: Response): Promise<void> {
        try {
            ...
            const subtemas =
                receita.correlacaoReceitas?.map(
                    (correlacao: Correlacao) => correlacao.subtema,
                ) || []
        ...
        try {
            ...
            if (req.body.subtema && Array.isArray(req.body.subtema)) {
                ...
                for (const subtema of req.body.subtema) {
                    ...
                    if (
                        temaSubtemaError &&
                        temaSubtemaError.code !== 'PGRST116'
                    )
                        throw temaSubtemaError
            ...
            const { data: receitaAtualizada, error: updateError } =
                await supabase
                    .from('receitas')
    ...
    async getAllVerifiedByTheme(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (idPostError)
                handleError(res, idPostError.message, 500, idPostError.details)
    ...
    async getReceitasPorSubtemas(req: Request, res: Response): Promise<void> {
        try {
            ...
            const idsReceitas = [
                ...new Set(
                    correlacoes.map((correlacao) => correlacao.idReceita),
                ),
            ]
